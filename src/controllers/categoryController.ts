...
export const category: any = async (req: Request,
    res: Response,
    next: NextFunction) => {
    try {
        ...
        if (existingCategory) {
            return res.json({ success: false, message: "Category with the same name already exists." });
        }
...
export const subCategory: any = async (req: Request,
    res: Response,
    next: NextFunction) => {
    try {
